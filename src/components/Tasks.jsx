...
const Tasks = ({ tasks, onAdd, onDelete, }) => {
    return <section>
        ...
        {tasks.length > 0 && <ul className="p-4 mt-8 bg-stone-100">
            {tasks.map((task) => <li className="flex justify-between my-4" key={task.id}><span>{task.text}</span><button onClick={()=>onDelete(task.id)} className="text-stone-700 hover:text-red500">Clear</button></li>)}
        </ul>}
